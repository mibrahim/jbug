...
public class SQL implements Closeable {
    ...
    void upgradeToV001() throws SQLException {
        ...
        queryNoRes(
                "CREATE TABLE bugs"
                        ...
                        + "GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1)," // Auto inc id
    ...
    private void upgradeToV003() throws SQLException {
        log.info("Upgrading to v003");

        queryNoRes("create index k01 on bugs(product, target_milestone)");
        setStringVar("dbversion", "3");

        log.info("Upgraded to V003");
        dbConnection.commit();
    }
